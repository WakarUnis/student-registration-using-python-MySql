...
class StuDet(Frame):
    ...
    def _enter_btn_clicked(self):
        ...
        if (len(stuName) == 0 or len(stuID) == 0 or len(stuDOB) == 0 or len(stuCourse) == 0):
            ...
            myCursor.execute(
                "INSERT INTO student_db(studentID, studentName, studentDOB, studentCourse) VALUES(%s, %s, %s, %s)",
                (stuID, stuName, stuDOB, stuCourse))
...
class UpdateDet(Frame):
    ...
    def _enter_btn_clicked(self):
        ...
        if (len(stuID) == 0):
            ...
            myCursor.execute(
                "UPDATE student_db SET studentName = %s, studentDOB = %s, studentCourse = %s WHERE StudentID = %s",
                (stuName, stuDOB, stuCourse, stuID))
